...
The frozen MycoMorph bundle pings the GitHub Releases API on startup; if
the latest tag is newer than the running ``mycomorph.__version__`` the
GUI shows a banner pointing to the release page. Running from source
(``python -m mycomorph`` in a venv) skips the check entirely so
...
GITHUB_API = "https://api.github.com/repos/timdewet/MycoMorph/releases/latest"
WINDOWS_ASSET_NAME = "MycoMorph-windows.zip"
...
class ReleaseInfo:
    ...
    asset_url: str       # direct download for MycoMorph-windows.zip ("" if missing)
...
class UpdateChecker(QObject):
    ...
    def check(self, *, force_in_dev: bool = False) -> None:
        ...
        req.setRawHeader(
            b"User-Agent", f"MycoMorph/{mycomorph.__version__}".encode("ascii", "replace")
        )
    ...
    def _on_finished(self) -> None:
        ...
        try:
            ...
            if _parse_version(version) <= _parse_version(mycomorph.__version__):
                self.upToDate.emit()
                return
