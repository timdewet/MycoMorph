...
def _replicate_strip_regex():
    ...
    Common patterns observed in MycoMorph labelling (current and likely-
    future variants):
    - ``"rpoB R1"`` → strip `` R1``
    - ``"rpoB_R1"`` → strip ``_R1``
...
def _run_umap_hdbscan(X_scaled, n_neighbors=3, min_dist=0.0,
    ...
    if X_scaled.size:
        ...
        # a local copy and warn via the mycomorph logger so it's visible
        ...
        if col_all_nan.any():
            import logging
            logging.getLogger("mycomorph").info(
...
def _morphology_cluster_plots(
    ...
    *,
    ...
    # default location (``~/.mycomorph/feature_library/``).
    try:
        from .feature_library import FeatureLibrary
        lib = FeatureLibrary(library_dir)
        df_lib = lib.load_species(species)
...
def _sinkhorn_divergence_matrix(
    ...
    # override via the env var ``MYCOMORPH_OT_JOBS`` if they want to
    # cap or expand parallelism explicitly.
    if n_jobs is None:
        env = os.environ.get("MYCOMORPH_OT_JOBS", "").strip()
        if env:
            try:
                n_jobs = int(env)
            except ValueError:
