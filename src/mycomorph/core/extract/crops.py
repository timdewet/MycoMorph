...
def derive_condition_fields(tiff_stem: str) -> dict[str, str]:
    """Map a MycoMorph per-well TIFF stem onto the MorphologicalProfiling_Mtb
    condition vocabulary.

    MycoMorph names per-well TIFFs as
...
def finalise_well_h5(h5_file, metadata: list[dict]) -> None:
    ...
    # MycoMorph-specific additions (additive — consumer ignores extras).
