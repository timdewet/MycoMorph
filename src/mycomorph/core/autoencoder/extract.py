...
def _harmony_correct(embeddings: np.ndarray, run_ids: np.ndarray) -> np.ndarray:
    ...
    Orientation handling is delegated to ``qc_plots._harmony_oriented``
    ...
    try:
        import harmonypy
        from mycomorph.core.extract.qc_plots import _harmony_oriented
        import numpy as np
        ...
        return _harmony_oriented(harmony, embeddings.shape[0])
    except ImportError:
        return embeddings
    except Exception:
        return embeddings
